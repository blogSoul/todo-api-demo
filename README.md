...
3. **실제 GitHub PR로 시연하기**: `main` 외에 `feature/todo-search`, `feature/comments-and-users` 브랜치가 미리 준비되어 있어 두 개의 PR로 나눠서 시연할 수 있습니다. 올리는 순서와 진행 방법은 `INSTRUCTOR_NOTES.md`를 참고하세요.
4. **오류·보안·성능 데모 (4-3 구간)**: 이 저장소에는 실전에서 자주 나오는 버그 3종이 의도적으로 심어져 있습니다. 정확한 위치와 시연 스크립트는 별도로 전달된 `INSTRUCTOR_NOTES.md` (강사용, 저장소에는 포함되지 않음)를 참고하세요.
